...
//                context.fill(barRect)
                let bezierPath = UIBezierPath(roundedRect: barRect, byRoundingCorners: [.topLeft, .topRight], cornerRadii: CGSize(width: barRect.width * set.barCornerRadiusFactor, height: barRect.width * set.barCornerRadiusFactor))
                context.addPath(bezierPath.cgPath)
                context.drawPath(using: .fill)
            }
        }
